...
def evaluate_model(model,test_loader,labels=None):
    ...
    for i, (X_test, lengths, y_test, ID) in tqdm(enumerate(test_loader)):
        y_pred = model(X_test, lengths)[0].softmax(dim=1).argmax(dim=1)
        y_tests = np.concatenate((y_tests,y_test.cpu().numpy()))
...
def show_metrics(y_tests,y_preds,label_names):
    print(classification_report(y_tests,y_preds,target_names=label_names),file=sys.stdout)
