    ...
    def train_model(self, model, epochs, train_loader, val_loader,  one_cycle=True,max_lr=1e-3, save=True, plot=True):
        ...
        for epoch in range(epochs):
            ...
            with tqdm(train_loader) as pbar:
                ...
                for i, (X_train, lengths, y_train, ids) in enumerate(pbar):
                    y_pred = model(X_train.to("cuda"), lengths, ids)[0]
            ...
            # Evaluate
            model.eval()
            for i, (X_val, lengths, y_val, ids) in enumerate(tqdm(val_loader)):
                y_pred = model(X_val.to("cuda"), lengths, ids)[0]
