...
class BertDictionary:
    def __init__(self, dataset, bert_model) -> None:
        self.embeddings = []
    ...
    def generate_embeddings(self):
        for i in range(len(self.dataset)):
            doc = self.dataset.__get_item__(i)[0]
            with torch.no_grad():
                embedded_doc_padded = self.bert_model(return_dict=True, **doc)["last_hidden_state"]
                self.embeddings.append(embedded_doc_padded)

    def get_embedding(self,index):
        return self.embeddings[i]


# Modified the code from https://github.com/JoungheeKim/Pytorch-Hierarchical-Attention-Network/blob/master/model.py
